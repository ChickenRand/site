...
$(function() {
  ...
  function setRngStatus(id, status, label) {
    ...
    $.post("/admin/set_rng_status/" + id + ".json", { status: ok }, function(
      data
    ) {});
  }
  ...
  function createRatioChart() {
    ...
    rngChart.addNumbersCb(function(data, rng) {
      ...
      $("#bit_rate").html(
        "Bitrate : " + Math.floor(bitRate / 1000) + "kbits/s"
      );
